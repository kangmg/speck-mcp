...
function create(): State {
  ...
  setCameraAngles(state, state.cameraTheta, state.cameraPhi);
  return state;
}
...
function center(state: State, structure: Structure) {
  ...
  for (const b of structure.cellBonds ?? []) {
    for (const p of [b.posA, b.posB]) {
      const r = 0.08;
      const v = vec4.fromValues(p.x, p.y, p.z, 0);
      vec4.transformMat4(v, v, state.rotation);

      maxX = Math.max(maxX, v[0] + r);
      minX = Math.min(minX, v[0] - r);
      maxY = Math.max(maxY, v[1] + r);
      minY = Math.min(minY, v[1] - r);
    }
  }
