...
export class Geometry {
  ...
  constructor(pico: App, structure: Structure, state: State) {
    ...
    if (state.bonds && structure.bonds.length > 0) {
      this.bondsVertexArray = this.createBondsVertexArray(
        structure.bonds.filter((bond) => bond.cutoff <= state.bondThreshold),
      );
    }

    if (state.cell && structure.cellBonds && structure.cellBonds.length > 0) {
      this.cellVertexArray = this.createBondsVertexArray(structure.cellBonds);
    }
  }
  ...
  createBondsVertexArray(bonds: Array<Bond>) {
    const imposter: number[] = [];
    const posa: number[] = [];
    const posb: number[] = [];
    const rada: number[] = [];
